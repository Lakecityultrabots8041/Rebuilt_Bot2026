...
public class IntakeSubsystems extends SubsystemBase {
  ...
  public void periodic() {
    ...
    if (pivotState != lastPivotState) {
      switch (pivotState) {
        case STOW -> pivotMotor1.setControl(
            motionMagicRequest.withPosition(IntakeConstants.STOW_POSITION));
        case INTAKE -> pivotMotor1.setControl(
            motionMagicRequest.withPosition(IntakeConstants.INTAKE_POSITION));
        case TRAVEL -> pivotMotor1.setControl(
            motionMagicRequest.withPosition(IntakeConstants.TRAVEL_POSITION));
  ...
  public Command intake() {
    return runOnce(() -> intakeState = IntakeState.INTAKING).withName("RunningIntakeMotor");
  }

  public Command eject() {
    return runOnce(() -> intakeState = IntakeState.EJECTING).withName("ReversingIntakeMotor");
  }

  public Command idle() {
    return runOnce(() -> intakeState = IntakeState.IDLE).withName("StoppingIntakeMotor");
  }

  // ===== PIVOT COMMANDS =====
  public Command pivotToStow() {
    return runOnce(() -> pivotState = PivotState.STOW).withName("PivotToStow");
  }

  public Command pivotToIntake() {
    return runOnce(() -> pivotState = PivotState.INTAKE).withName("PivotToIntake");
  }

  public Command pivotToTravel() {
    return runOnce(() -> pivotState = PivotState.TRAVEL).withName("PivotToTravel");
  }

  /** Release the pivot motor so the arm can bounce freely on the bumper. */
  public Command pivotIdle() {
    return runOnce(() -> pivotState = PivotState.IDLE).withName("PivotIdle");
  }
  ...
  public TalonFX getPivotMotor1() {
    return pivotMotor1;
  }

  public TalonFX getPivotMotor2() {
    return pivotMotor2;
  }
}
