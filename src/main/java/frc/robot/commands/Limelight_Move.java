...
/**
 * Aligns to any visible AprilTag in a tag group. Tag group resolves at command start so alliance
 * detection works automatically. See vision.md.
 */
public class Limelight_Move extends Command {
  ...
  public void execute() {
    ...
    forwardOutput =
        MathUtil.clamp(
            forwardOutput, -VisionConstants.MAX_FORWARD_SPEED, VisionConstants.MAX_FORWARD_SPEED);
    ...
    if (Math.abs(driverStrafe) > 0.05) {
      strafeVelocityMps =
          -driverStrafe
              * maxSpeedMps
              * VisionConstants.MAX_DRIVER_STRAFE_SCALE
              * directionMultiplier;
    } /* else {
    ...
    SmartDashboard.putNumber(
        "Vision/Target Distance (in)", Units.metersToInches(targetDistanceMeters));
    SmartDashboard.putNumber(
        "Vision/Distance Error (in)", Units.metersToInches(distanceErrorMeters));
    ...
    SmartDashboard.putNumber(
        "Vision/Lateral Offset (in)", Units.metersToInches(lateralOffsetMeters));
    ...
    boolean rotationGood =
        Math.abs(horizontalErrorDeg) < VisionConstants.ALIGNMENT_TOLERANCE_DEGREES;
    boolean distanceGood =
        Math.abs(distanceErrorMeters) < VisionConstants.DISTANCE_TOLERANCE_METERS;
