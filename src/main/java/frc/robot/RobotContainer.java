...
public class RobotContainer {
  ...
  // Intake camera ("limelight-intake"), faces the intake side. Provides rear vision and pose
  // fusion.
  ...
  private Limelight_Move createHubAlign() {
    return new Limelight_Move(
        drivetrain, limelightShooter, VisionConstants::getHubTags, () -> -controller.getLeftX());
  }

  private Limelight_Move createTowerAlign() {
    return new Limelight_Move(
        drivetrain, limelightShooter, VisionConstants::getTowerTags, () -> -controller.getLeftX());
  }
  ...
  private Limelight_Move createOutpostAlign() {
    return new Limelight_Move(
        drivetrain,
        limelightShooter,
        VisionConstants::getOutpostTags,
        () -> -controller.getLeftX());
  }
  ...
  public RobotContainer() {
    ...
    limelightShooter.setSpinRateSupplier(() -> drivetrain.getState().Speeds.omegaRadiansPerSecond);

    limelightIntake.setRobotPoseSupplier(() -> drivetrain.getState().Pose);
    limelightIntake.setVisionPoseConsumer(
        (pose, timestamp, accuracy) -> drivetrain.addVisionMeasurement(pose, timestamp, accuracy));
    limelightIntake.setSpinRateSupplier(() -> drivetrain.getState().Speeds.omegaRadiansPerSecond);
    ...
    NamedCommands.registerCommand(
        "AlignHubAndShoot",
        Commands.sequence(
            createAutoHubAlign().andThen(ShooterCommands.quickShoot(shooterSubsystem))));
    NamedCommands.registerCommand(
        "AlignOutpostAndShoot",
        Commands.sequence(
            createAutoOutpostAlign().andThen(ShooterCommands.shootSequence(shooterSubsystem))));
    NamedCommands.registerCommand(
        "AlignTowerAndShoot",
        Commands.sequence(
            createAutoTowerAlign().andThen(ShooterCommands.shootSequence(shooterSubsystem))));
    // =====Intake Commands=====
    ...
    // IntakeCommands.startingIntakeSequence(intakeSubsystem));
    // NamedCommands.registerCommand("End Intake",
    // IntakeCommands.endingIntakeSequence(intakeSubsystem));
  ...
  private void configureBindings() {
    ...
    drivetrain.setDefaultCommand(
        drivetrain.applyRequest(
            () -> {
              currentDriveX =
                  applyDriveSlew(currentDriveX, -controller.getLeftY() * MaxSpeed * 0.9);
              currentDriveY =
                  applyDriveSlew(currentDriveY, -controller.getLeftX() * MaxSpeed * 0.9);
              ...
              if (autoAimEnabled && limelightShooter.isTrackingHubTag()) {
                ...
                double targetHeading =
                    currentHeading - Math.toRadians(limelightShooter.getHorizontalOffset());
                ...
                if (distance > 0 && !controller.rightTrigger().getAsBoolean()) {
                  shooterSubsystem.setAutoAimSpeed(ShooterConstants.getSpeedForDistance(distance));
                }
    ...
    */
    ...
    controller
        .rightTrigger()
        .whileTrue(ShooterCommands.shoot(shooterSubsystem))
        .onFalse(ShooterCommands.idle(shooterSubsystem));
    controller
        .x()
        .whileTrue(ShooterCommands.eject(shooterSubsystem))
        .onFalse(ShooterCommands.idle(shooterSubsystem));
    ...
    controller
        .leftTrigger()
        .whileTrue(IntakeCommands.intake(intakeSubsystem))
  ...
  /**
   * Asymmetric slew rate limiter. Accelerating (|requested| > |current|) uses MAX_TELEOP_ACCEL.
   * Decelerating or reversing uses MAX_TELEOP_DECEL. Tune both values in DriveConstants.java.
   */
  private double applyDriveSlew(double current, double requested) {
    double rateLimit =
        (Math.abs(requested) > Math.abs(current))
  ...
  public void updateDriverDashboard() {
    boolean shooterReady =
        shooterSubsystem.atTargetVelocity() && shooterSubsystem.atFlywheelTargetVelocity();
    boolean visionLocked = autoAimEnabled && limelightShooter.isTrackingHubTag();
    boolean intakeDown =
        intakeSubsystem.getPivotState() == IntakeSubsystems.PivotState.INTAKE
  ...
  public LimelightSubsystem getLimelightShooter() {
    return limelightShooter;
  }

  public LimelightSubsystem getLimelightIntake() {
  ...
  public IntakeSubsystems getIntake() {
