...
Verify steps against current Limelight documentation at **docs.limelightvision.io** — this process can change between seasons.

---

## What You're Actually Doing

The Hailo chip runs a tiny neural network that looks at every camera frame and draws a box around anything it thinks is fuel. You train the network by showing it hundreds of photos of fuel labeled with boxes, and it learns the pattern. Once trained, the network runs entirely on the Limelight — no RoboRIO processing needed.
...
This is the most important step. **More images = more accurate model. Diverse images = more robust model.**
...
Retraining with more targeted images fixes most accuracy problems. It usually takes 2–3 rounds of collect → label → train → test before a model is competition-ready.
