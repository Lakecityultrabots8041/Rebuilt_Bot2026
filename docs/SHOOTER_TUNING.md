...
The key insight is that the **difference in speed between the two wheels** is what puts spin on the fuel, and spin is what creates the arc.
...
The `FLYWHEEL_SPEED_RATIO` in ShooterConstants controls how much faster the flywheel runs relative to the shooter in auto-aim mode. The preset velocities (`FLYWHEEL_MAX_VELOCITY` vs `MAX_VELOCITY`) control the same thing for button-triggered shots.
...
| `FLYWHEEL_SPEED_RATIO` | 1.15 | Flywheel multiplier in auto-aim mode (flywheel = shooter × ratio) |
...
- **Shot too flat / hits the rim low** → increase `FLYWHEEL_MAX_VELOCITY` (or increase `FLYWHEEL_SPEED_RATIO` for auto-aim shots)
- **Shot too high / arcs over the target** → decrease `FLYWHEEL_MAX_VELOCITY`
- **Shot not consistent** → check PIDs first (section below), then revisit velocities

A 10–20% flywheel advantage is a typical starting range. Current defaults are ~16.7% (`105 / 90`).

### Step 3 — Distance-based velocity (auto-aim mode)

In auto-aim, shooter velocity is interpolated from the distance table and flywheel = shooter × `FLYWHEEL_SPEED_RATIO`. The table is in ShooterConstants:
...
The shooter uses `VelocityTorqueCurrentFOC` — torque-current based closed-loop velocity control. Three gains matter:
...
**kS** — Start at 0. Increase until the motor reliably starts spinning from rest without hesitation. Too high = motor jerks on startup. Typical: 0.1–0.5 A.

**kV** — Set kP = 0 first. Increase kV until the motor runs close to the target RPS at steady state. Watch `Shooter/Actual RPS` vs `Shooter/Commanded RPS` on SmartDashboard. Too low = motor runs slow, too high = motor runs fast. Typical: 0.1–0.2 A/RPS.

**kP** — Re-enable kP. Increase from 0 until steady-state error closes. Too high = oscillation (the RPS bounces around the target). Too low = motor sits 3–5 RPS below target. Typical: 1.0–5.0.
...
| `Shooter/Motor Current` | Excessive current (>60A sustained) = PID too aggressive or gains too high |

---

## Passing Shot Setup

The passing command (`B button`) spins up to `PASS_VELOCITY` / `FLYWHEEL_PASS_VELOCITY` instead of the normal scoring presets. These are intentionally set lower than scoring maximums because for a long-distance pass you typically want a flatter, faster trajectory — less arc, more range.
...
3. Release **B button** — shooter idles. (Or bind differently if you want to hold and fire separately — discuss with driver)
...
| Shot arcs too high | Decrease `FLYWHEEL_MAX_VELOCITY` (close the ratio gap) |
| Shot arcs too low / hits rim | Increase `FLYWHEEL_MAX_VELOCITY` (widen the ratio gap) |
...
| Pass too flat / no arc | Increase `FLYWHEEL_PASS_VELOCITY` |
